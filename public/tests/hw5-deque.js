...
describe('makeDeque', function() {
  ...
  var testDeck;
  function getArray(deque) { // used to get array contents from IIFE
    var foundArray = [];
    for( var i = 0; i < deque.length(); i++) {
      var firstItem = deque.shift();
      foundArray.push(firstItem);
      deque.push(firstItem);
    }
    return foundArray;
  }
  ...
  it('should not have an accessible array', function() {
    assert.isUndefined(testDeck.array, inputArray);
  });
  ...
  describe('instance.pop', function() {
    ...
    it('should remove the returned item from the array', function() {
      assert.deepEqual(getArray(testDeck), ['first', 'second', 'third', 'fourth', 'fifth', 'sixth', 'seventh', 'eigth', 'ninth']);
    });
  ...
  describe('instance.push', function() {
    ...
    it('should add an item to the end (or "top") of the deque', function() {
      assert.deepEqual(getArray(testDeck), ['first', 'second', 'third', 'fourth', 'fifth', 'sixth', 'seventh', 'eigth', 'ninth', 'tenth']);
    });
  ...
  describe('instance.shift', function() {
    ...
    it('should remove the returned item from the array', function() {
      assert.deepEqual(getArray(testDeck), ['second', 'third', 'fourth', 'fifth', 'sixth', 'seventh', 'eigth', 'ninth', 'tenth']);
    });
    it('should decrease the length of the deque by one', function() {
      assert.equal(testDeck.length(), 9);
    });
  ...
  describe('instance.cut', function() {
    ...
    it('should put the second half of the deque in front of the first half', function() {
      testDeck.cut();
      assert.deepEqual(getArray(testDeck), ['sixth', 'seventh', 'eigth', 'ninth', 'tenth','first', 'second', 'third', 'fourth', 'fifth']);
    });
    it('should split towards the end (or "top") of the array if the array length it odd', function() {
      testDeck.pop();
      testDeck.cut();
      assert.deepEqual(getArray(testDeck), ['first', 'second', 'third', 'fourth','sixth', 'seventh', 'eigth', 'ninth','tenth']);
    });
    it('should not change deques with fewer than two items', function() {
      var singleItemDeque = makeDeque([1]);
      singleItemDeque.cut();
      assert.deepEqual(getArray(singleItemDeque), [1]);
    });
  });
  describe('instance.sort', function() {
    ...
    it('should sort the deque in lexical order if no arguments are given', function() {
      testDeck.sort();
      assert.deepEqual(getArray(testDeck), ['eigth', 'first', 'fourth', 'ninth', 'second', 'seventh', 'sixth', 'tenth','third']);
    });
    it('should sort the deque according to the provided function if one is given', function() {
      ...
      assert.deepEqual(getArray(testDeck), ['third', 'tenth', 'sixth', 'seventh', 'second', 'ninth', 'fourth', 'first', 'eigth']);
    });
  });
  ...
  describe('instance.shuffle', function() {
    ...
    it('should not add to or remove from deque\'s array', function() {
      testDeck.shuffle();
      assert.sameMembers(getArray(testDeck), ['third', 'tenth', 'sixth', 'seventh', 'second', 'ninth', 'fourth', 'first', 'eigth']);
    });
    it('should change the order of the contents of the array', function() {
      // could give a false negative
      assert.notDeepEqual(getArray(testDeck), ['third', 'tenth', 'sixth', 'seventh', 'second', 'ninth', 'fourth', 'first', 'eigth']);
    });
  });
  describe('Deque security', function() {
    it('should allow previously included items to be added back to the deque', function() {
      testDeck.push('fifth');
      assert.sameMembers(getArray(testDeck), ['first', 'second', 'third', 'fourth', 'fifth', 'sixth', 'seventh', 'eigth', 'ninth', 'tenth']);
    });
...
describe('Deque rendering functions', function() {
  var inputArray = ['first', 'second', 'third', 'fourth', 'fifth', 'sixth', 'seventh', 'eigth', 'ninth', 'tenth'];
  var testDeck = makeDeque(inputArray);
  it('should be a function', function() {
    assert.isFunction(testDeck.render);
  });
  var $sandbox = $('<div />').attr('id', 'sandbox').hide().appendTo('body');
  testDeck.render('sandbox', function(item, cell) { $(cell).html('<span>' + item + '</span'); });
  it('should create and append a div for each element in the array to the passed-in container element', function() {
    assert.lengthOf( $sandbox.children(), testDeck.length() );
  });
  it('should run the provided callback function for each item in the deque, using the item and the created div as parameters', function() {
    assert.equal( $sandbox.children().html(), '<span>first</span>' );
  });
});
